...
class ProfileController extends MemberbaseController {
    ...
    public  function mydynamic(){
        ...
        $where['status']=array(array('eq',1),array('exp','IS NULL'),'OR');
            # code...
        $where['userid']=array(array('eq',session('user.id')));
        ...
        $res=M('dynamic')->where($where)->limit($page->firstRow , $page->listRows)->select();
        
        $this->assign("page", $page->show('Admin'));
        $this->assign("res",$res);
        $this->display(":mydynamic");
    }
    ...
    public  function dodynamic(){
        ...
        if($res){
            $this->success("发布成功",U('user/profile/mydynamic'));
        }else{
            $this->error("发布失败");
        }
    }
    ...
    public  function editdynamic(){
        if(IS_POST){
            ...
            $did=I('get.did');
            $res=M("dynamic")->where("did=$did")->find();
            $this->assign("res",$res);
